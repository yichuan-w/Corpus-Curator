...
nq = load_dataset("Tevatron/wikipedia-trivia")

queries = []
# for example in nq['train']:
#     queries.append(example['query'] + " " + " ".join(example['answers']))
...
doc_ids = set()
with open('runs/run.bm25-train.trivia.trec', 'w') as f_train, open('runs/run.bm25-test.trivia.trec', 'w') as f_test, open('runs/run.bm25-dev.trivia.trec', 'w') as f_dev:
